    ...
    float dt, int timesteps, System &sys, Dump &dump, Thermo &thermo, std::vector<Fix> &fixes) 
{
    ...
    h_f = (float *) calloc(sys.natoms*3, sizeof(float));
    ...
    for (int ti = 0; ti < timesteps; ti++) {
        ...
        for (std::vector<Fix>::iterator ifix = fixes.begin(); ifix != fixes.end(); ifix++) {
            for (int a = 0; a < sys.natoms; a++) {
                int a3 = a * 3;
                if (sys.type[a] == ifix->type) {
                    sys.x[a3] += ifix->dispx * dt;
                    sys.x[a3 + 1] += ifix->dispy * dt;
                    sys.x[a3 + 2] += ifix->dispz * dt;
        ...
        for (int a = 0; a < sys.natoms; a++) {
            ...
            sys.v[a3] += sys.a[a3] * dt/2.0;
            sys.v[a3 + 1] += sys.a[a3 + 1] * dt/2.0;
            sys.v[a3 + 2] += sys.a[a3 + 2] * dt/2.0;
        }
        ...
        for (int a = 0; a < sys.natoms; a++) {
            ...
            sys.x[a3] += sys.v[a3] * dt;
            sys.x[a3 + 1] += sys.v[a3 + 1] * dt;
            sys.x[a3 + 2] += sys.v[a3 + 2] * dt;
        }
        ...
        for (int b = 0; b < sys.nbonds; b++) {
            int ai3 = sys.atom_i[b] * 3;
            int aj3 = sys.atom_j[b] * 3;

            double r_ij = sqrt(pow(sys.x[aj3] - sys.x[ai3], 2) + 
                          pow(sys.x[aj3 + 1] - sys.x[ai3 + 1], 2) + 
                          pow(sys.x[aj3 + 2] - sys.x[ai3 + 2], 2));

            if (r_ij > h_rc[b]) h_k[b] = 0;

            if (r_ij < EPS) r_ij += EPS;

            double fix = -h_k[b] * (r_ij - h_r0[b]) * (sys.x[ai3] - sys.x[aj3])/r_ij;
            double fiy = -h_k[b] * (r_ij - h_r0[b]) * (sys.x[ai3 + 1] - sys.x[aj3 + 1])/r_ij;
            double fiz = -h_k[b] * (r_ij - h_r0[b]) * (sys.x[ai3 + 2] - sys.x[aj3 + 2])/r_ij;
        ...
        for (int a = 0; a < sys.natoms; a++) {
            int a3 = a * 3;
            sys.a[a3] = h_f[a3]/h_m[a];
            sys.a[a3 + 1] = h_f[a3 + 1]/h_m[a];
            sys.a[a3 + 2] = h_f[a3 + 2]/h_m[a];
        }
        ...
        if (ti % dump.ndump == 0 || ti % thermo.nthermo == 0) {
            ...
            if (ti % dump.ndump == 0) {
                ...
                for (int a = 0; a < sys.natoms; a++) {
                    ...
                    h_stress[a6] -= h_m[a] * sys.v[a3] * sys.v[a3];
                    h_stress[a6 + 1] -= h_m[a] * sys.v[a3 + 1] * sys.v[a3 + 1];
                    h_stress[a6 + 2] -= h_m[a] * sys.v[a3 + 2] * sys.v[a3 + 2];
                    h_stress[a6 + 3] -= h_m[a] * sys.v[a3] * sys.v[a3 + 1];
                    h_stress[a6 + 4] -= h_m[a] * sys.v[a3 + 1] * sys.v[a3 + 2];
                    h_stress[a6 + 5] -= h_m[a] * sys.v[a3 + 2] * sys.v[a3];
                }                
                ...
                for (int b = 0; b < sys.nbonds; b++) {
                    ...
                    int ai6 = sys.atom_i[b] * 6;
                    int aj6 = sys.atom_j[b] * 6;
                    ...
                    h_stress[ai6] += 1.0 / 2.0 * (sys.x[aj3] - sys.x[ai3]) * fix;  // sxx
                    h_stress[ai6 + 1] += 1.0 / 2.0 * (sys.x[aj3 + 1] - sys.x[ai3 + 1]) * fiy;  // syy
                    h_stress[ai6 + 2] += 1.0 / 2.0 * (sys.x[aj3 + 2] - sys.x[ai3 + 2]) * fiz;  // szz
                    h_stress[ai6 + 3] += 1.0 / 2.0 * (sys.x[aj3] - sys.x[ai3]) * fiy;  // sxy
                    h_stress[ai6 + 4] += 1.0 / 2.0 * (sys.x[aj3 + 1] - sys.x[ai3 + 1]) * fiz;  // syz
                    h_stress[ai6 + 5] += 1.0 / 2.0 * (sys.x[aj3 + 2] - sys.x[ai3 + 2]) * fix;  // szx

                    h_stress[aj6] += 1.0 / 2.0 * (sys.x[aj3] - sys.x[ai3]) * fix;  // sxx
                    h_stress[aj6 + 1] += 1.0 / 2.0 * (sys.x[aj3 + 1] - sys.x[ai3 + 1]) * fiy;  // syy
                    h_stress[aj6 + 2] += 1.0 / 2.0 * (sys.x[aj3 + 2] - sys.x[ai3 + 2]) * fiz;  // szz
                    h_stress[aj6 + 3] += 1.0 / 2.0 * (sys.x[aj3] - sys.x[ai3]) * fiy;  // sxy
                    h_stress[aj6 + 4] += 1.0 / 2.0 * (sys.x[aj3 + 1] - sys.x[ai3 + 1]) * fiz;  // syz
                    h_stress[aj6 + 5] += 1.0 / 2.0 * (sys.x[aj3 + 2] - sys.x[ai3 + 2]) * fix;  // szx
